...
@Controller('info_mixers')
export class InfoMixersController {

    constructor(private i_service: InfoMixersService) {
    ...
    @Post('add')
    async addItem(@Body() createUserOto: InfoMixersEntity){
    ...
    @Post("bulk")
    async createBulk(@Body() createUserOto: InfoMixersEntity[]) {
      try {
